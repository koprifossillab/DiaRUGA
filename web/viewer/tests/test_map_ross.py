...
class MapPointsFallbackTest(DiaRUGATestCase):
...
class RossContextTest(DiaRUGATestCase):
...
class RossRenderTest(DiaRUGATestCase):
    ...
    def test_한국_화면에는_확대가_없다(self):
        ...
        html = self.html("kr")
        # CSS 에는 `.rossmap` 이 늘 있다 — 요소를 짚는다
        self.assertNotIn('class="antmap rossmap"', html)
        self.assertNotIn('id="mapzoom"', html)
