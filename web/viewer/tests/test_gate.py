...
class GateOffTests(DiaRUGATestCase):
...
class GateOnTests(DiaRUGATestCase):
...
@override_settings(GATE_CODE=CODE, FORCE_SCRIPT_NAME="/DiaRUGA")
class GateSubpathTests(DiaRUGATestCase):
    """운영은 `/DiaRUGA/` 아래에 있다 — 쿠키 경로와 되돌아갈 주소가 그 안이어야 한다."""

    def setUp(self):
        # 운영의 WSGIHandler 는 요청마다 접두사를 잡는데 시험 클라이언트는 안 잡는다
        set_script_prefix("/DiaRUGA/")
        self.addCleanup(clear_script_prefix)

    def test_서브경로(self):
        c = Client()
        r = c.get("/", SCRIPT_NAME="/DiaRUGA")
        self.assertTrue(r["Location"].startswith("/DiaRUGA/gate/?next=%2FDiaRUGA%2F"),
                        r["Location"])
        r = c.post("/gate/", {"code": CODE, "next": "/DiaRUGA/"}, SCRIPT_NAME="/DiaRUGA")
        self.assertEqual(r.cookies[gate.COOKIE]["path"], "/DiaRUGA/")
        self.assertEqual(r["Location"], "/DiaRUGA/")
