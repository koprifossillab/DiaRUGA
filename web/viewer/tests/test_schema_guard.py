...
class CompareTest(DiaRUGATestCase):
    ...
    def test_칼럼이_걷히면_잡는다(self):
        ...
        with connection.cursor() as cur:
            cur.execute("ALTER TABLE viewer_objectreview DROP COLUMN bind_score")
        tables, cols = schema_guard.compare()
        self.assertEqual(tables, [])
        self.assertEqual(cols, ["viewer_objectreview.bind_score"])

    def test_테이블이_걷히면_잡는다(self):
        """모델 이름을 바꾼 경우(`Core` → `Locality` · 063)가 이 모양이다."""
        with connection.cursor() as cur:
            cur.execute("PRAGMA foreign_keys=OFF")
            cur.execute("DROP TABLE viewer_taxonname")
        tables, cols = schema_guard.compare()
        self.assertIn("viewer_taxonname", tables)
...
class CheckOrExitTest(DiaRUGATestCase):
    """어긋나면 3 으로 끝내고 **깃발을 세운다** — `/healthz` 가 그것을 읽는다."""

    def setUp(self):
        super().setUp()
        self.tmp = tempfile.TemporaryDirectory()
        self.db = Path(self.tmp.name) / "DiaRUGA.db"
        self.db.touch()
    ...
    def test_같으면_지나가고_자기_깃발을_지운다(self):
        ...
        with override_settings(DIARUGA_DB=self.db):
    ...
    def test_어긋나면_3_으로_끝내고_깃발을_세운다(self):
        ...
        self.assertIn("segment_diatoms", flags[0]["reason"])
        self.assertIn("viewer_objectreview.bind_score", flags[0]["reason"])
