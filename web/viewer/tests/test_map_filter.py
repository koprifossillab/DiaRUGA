...
from .test_detection_js import js_blocks
from .test_map_ross import CORES
from ..models import Locality, Site


class MapFilterRenderTest(DiaRUGATestCase):
    ...
    def setUpTestData(cls):
        ...
        fx.make_world(slug="bp", area="kr", kind="outcrop", site_code="BP",
                      loc_code="BP09", sample_code="0901", with_files=False)
        Site.objects.filter(code="BP").update(lat=37.5, lon=129.1)
    ...
    def test_한국_지도에도_붙는다(self):
        html = self.html("kr")
        self.assertIn('id="mapfilter"', html)
        self.assertIn('data-site="BP">', html)
        self.assertIn('data-slug="bp" data-vp=', html)
        self.assertIn('class="msite" data-site="BP"', html)

    @unittest.skipUnless(shutil.which("node"), "node 가 없다")
    def test_인라인_스크립트가_파싱된다(self):
        blocks = [b for b in js_blocks(self.html("ant")) if "diaruga.maphide" in b]
