...
**켜고 끄는 것은 `DIARUGA_GATE_CODE` 하나다.** 비어 있으면 아무 일도 안 한다 —
시험·개발 서버·코드를 아직 안 정한 배포가 그 갈래다. 값이 있으면 코드를 모르는
요청은 전부 잠금 화면(`/gate/`)으로 간다.

## 한 번만 묻는 방법

맞는 코드를 넣으면 **서명한 쿠키**(`diaruga_gate`)를 준다. 안에는 코드 자체가
아니라 코드의 지문과 발급 시각이 들어 있고, `DIARUGA_SECRET_KEY` 로 서명한다.
그래서

- **코드를 바꾸면 이전 쿠키가 전부 무효가 된다** — 지문이 달라진다. 모두에게
  다시 묻고 싶을 때 코드를 바꾸면 된다.
- **`DIARUGA_SECRET_KEY` 를 바꿔도 전부 무효가 된다** — 서명이 안 맞는다.
...
잠그면 **배포가 스스로 멈춘다**(034 와 같은 이야기다). 잠금 화면과 그 그림 둘도
당연히 열려 있다.
...
# 잠그지 않는 주소 (path_info — 서브경로 `/DiaRUGA` 를 뗀 것)
...
def _fingerprint(code):
    return hashlib.sha256(f"diaruga-gate:{code}".encode()).hexdigest()[:24]
...
def _give_cookie(resp):
    resp.set_cookie(
        ...
        # 서브경로마다 따로다 — 운영(/DiaRUGA/)과 시험(/DiaRUGATest/)이 한 오리진에
        # 떠 있어도 서로의 쿠키를 안 쓴다
