...
**실행은 [208](20260918_208_biodatum-layer.md)** — 같은 날 1~3단계를 했다.
§7 의 답은 208 머리에 있다.
...
3. **재인용이 원문 직접 파싱과 같은 출처키로 앉아 있다.** `cody2008` 460행
   ...
   두 번 찍으면 안 된다. (208 에서 실측: **모델까지 같은 것은 41행**이다 —
   LO *H. karstenii* 는 연령은 같은데 평균 모델이라 적고 전범위 값을 옮긴
   것이라 겹침이 아니라 재인용 오차 쪽이다)
...
**답을 받았다 (2026-09-18, 208 머리에 그대로 옮겼다)** — 1 옮긴다 · 2 합쳐도
된다 · 3 합치지 않는다, 반입 뒤 전부 AlgaeBase 재확인, 도판 논문의 확인된
종은 뺀 목록을 만든다 · 4 넣고 숨기되 참고문헌은 남긴다 · 5·6 은 계획대로 ·
**권역 거르개는 체크박스**(태평양을 남극과 함께 볼 때가 있다).
