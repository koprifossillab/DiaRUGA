...
```bash
python web/manage.py test viewer --exclude-tag browser   # 997개 · 19초
python web/manage.py test viewer                         # 1,239개 (브라우저 242개 포함, 490초)
```
...
- **그런데 스키마를 조이는 마이그레이션은 둘을 함께 올려야 한다.** 갈라 놓은
  ...
  볼 것** (055 에서 그렇게 잡았다). **사람의 실측은 갈래를 놓친다** — 114 가
